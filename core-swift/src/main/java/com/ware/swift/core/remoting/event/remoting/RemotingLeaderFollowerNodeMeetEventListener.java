 ...
 */
public class RemotingLeaderFollowerNodeMeetEventListener extends AbstractRemotingPipelineEventListener {

    public RemotingLeaderFollowerNodeMeetEventListener() {
    }
    ...
    public boolean onEvent(ObjectEvent<IInteractive> event, int listenerIndex) {
        try {
            ...
            // 启动数据节点复制的标识，将会在下一次心跳的时候检测到这个开关，将会通知当前新连接过来的节点进行数据的复制。
