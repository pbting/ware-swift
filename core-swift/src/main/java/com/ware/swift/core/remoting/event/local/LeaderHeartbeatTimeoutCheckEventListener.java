    ...
    public boolean onEvent(ObjectEvent<IMailbox<InteractivePayload>> event,
        ...
        if (heartbeatPayload != null) {
            logger.debug(String.format(
                    "cost time [%s] ms to wait leader[%s] send information with heartbeat.",
                    (System.currentTimeMillis() - s), heartbeatPayload.getSource()));
        ...
        if (remotingManager.getWareSwiftConfig().getLeader().identify().equalsIgnoreCase(
        ...
        try {
            InteractivePayload responsePayload = leaderChannel
                    .requestResponse(InteractivePayload.newBuilder().setEventType(
                            ...
                            .build());
            logger.info(String.format("double check leader is contains %s:%s",
            ...
            if (!Boolean.valueOf(responsePayload.getPayload().toStringUtf8())) {
                ...
                LeaderFollowerSendSayHelloEventListener.sendSayHello(leaderChannel,
                        remotingManager.getChannelIdentify(remotingManager.getWareSwiftConfig()
