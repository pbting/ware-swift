...
public final class RemotingEventDispatcher
    ...
    private RemotingEventDispatcher() {
        super(new FastParallelActionExecutor(
                Runtime.getRuntime().availableProcessors() * 2 + 1,
                "ware-swift-remoting-event-executor"), true);
    }
     ...
     * 广播 leader 给其他节点
     */
    public static final int REMOTING_BROADCAST_LEADER_EVENT_TYPE = 0x03;

    /**
     * 处理接收来自 Leader 发送的心跳
     */
    public static final int REMOTING_RECEIVE_HEART_BEAT_EVENT_TYPE = 0x04;

    /**
     * 处理接收来自去中心架构下发送的心跳
     */
    public static final int REMOTING_RECEIVE_DECENTRA_HEARTBEAT_EVENT_TYPE = 0x05;
     ...
     */
    public static final int REMOTING_LEADER_NODE_MEET_EVENT_TYPE = 0x08;
     ...
     *
     */
    public static final int REMOTING_CONTROLLER_BACKLOG_QUEUE_EVENT_TYPE = 0x11;
    ...
    public void attachListener() {
        ...
        this.addLast(new RemotingReceiveLeaderElectionEventListener(),
                REMOTING_BROADCAST_LEADER_EVENT_TYPE);
        this.addLast(new RemotingReceiveLeaderHeartbeatEventListener(),
                REMOTING_RECEIVE_HEART_BEAT_EVENT_TYPE);
        this.addLast(new RemotingReceivePeerHeartbeatEventListener(),
                REMOTING_RECEIVE_DECENTRA_HEARTBEAT_EVENT_TYPE);
        this.addLast(new RemotingReceiveSdownLeaderEventListener(),
                REMOTING_RECEIVE_SDOWN_LEADER_EVENT_TYPE);
        this.addFirst(new RemotingLeaderFollowerNodeMeetEventListener(),
                REMOTING_LEADER_NODE_MEET_EVENT_TYPE);
        ...
        this.addLast(new RemotingDecentralizeNodeMeetEventListener(),
                REMOTING_DECENTRATION_NODE_MEET_EVENT_TYPE);
        this.addLast(new RemotingDataStreamSyncEventListener(),
                REMOTING_STREAM_DATA_SYNC_EVENT_TYPE);
        this.addLast(new RemotingRuntimeDataSyncEventListener(),
                REMOTING_DATA_SYNCING_EVENT_TYPE);
        this.addLast(new RemotingGetCommittedCountEventListener(),
                REMOTING_GET_COMMITTED_EVENT_TYPE);
        this.addLast(new RemotingControllerBacklogQueueEventListener(),
                REMOTING_CONTROLLER_BACKLOG_QUEUE_EVENT_TYPE);
