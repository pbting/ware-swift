...
public class BroadcastNewNodeEventListener
    ...
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(BroadcastNewNodeEventListener.class);
    ...
    public boolean onEvent(ObjectEvent<NodeInformation> event, int listenerIndex) {
        ...
        try {
            ...
            logger.info("response broadcast new node:"
                    + response.getPayload().toStringUtf8());
        } catch (Exception e) {
            if (remotingChannel.isNetUnavailable(e)) {
                // 等待下一秒发送，直到 channel 可用为止。
                logger.error(remotingChannel.identify()
                        + " net unavailable for ...." + this.getClass().getName(),e);
            }
            return false;
        }
        return true;
