...
public class BroadcastVoteCountEventListener
    ...
    public boolean onEvent(ObjectEvent<WareCoreSwiftGlobalContext> event, int listenerIndex) {
        ...
        if (remotingManager.getActiveChannelCount() == 0 || remotingManager
                .getRaftConfig().getNodeInformation()
                .getVoteCount() > (remotingManager.getActiveChannelCount()) / 2 + 1) {
            event.setInterruptor(true);
            WareCoreSwiftConfig raftConfig = remotingManager.getRaftConfig();
            raftConfig.getNodeInformation().increTerm();
            raftConfig.setLeader(remotingManager.getRaftConfig().getNodeInformation());
            logger.debug(remotingManager.getRaftConfig().getNodeInformation().identify()
