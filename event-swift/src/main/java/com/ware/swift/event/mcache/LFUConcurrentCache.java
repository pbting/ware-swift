...
public class LFUConcurrentCache<K, V> extends AbstractConcurrentCache<K, V> {
    ...
    //定义用来存放访问次数Set集合
    private Set<Integer> accessCountSort = new ConcurrentSkipListSet<>(new Comparator<Integer>() {
     ...
     * key和访问次数据映射的hashmap
     */
    private ConcurrentHashMap<K, Integer> keyAccessCount = new ConcurrentHashMap<>();

    /**
     * 访问映射字段：次数和这一类key的映射排序分类处理
     */
    private ConcurrentHashMap<Integer, LinkedBlockingQueue<K>> accessCountQueue = new ConcurrentHashMap<>();
