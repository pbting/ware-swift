...
public class DisruptorParallelQueueExecutor extends AbstractParallelQueueExecutor {
    ...
    private Disruptor<RunnableEvent>[] disruptorArray;
    ...
    public void execute(String topic, Runnable command) {
        ...
        Disruptor<RunnableEvent> ringBuffer = disruptorArray[index];
    ...
    public void stop() {
        ...
        for (Disruptor<RunnableEvent> disruptor : disruptorArray) {
            disruptor.shutdown();
        }
    ...
    public void executeOneTime(Runnable command) {
        Disruptor<RunnableEvent> ringBuffer = disruptorArray[ringBufferExecutorIndexChooser
                .allocator()];
        runing(ringBuffer, command);
    }
}
