...
public class FastParallelActionExecutor extends AbstractParallelActionExecutor {
    ...
    // 维护一个 可以水平缩放的并行队列。并行队列的大小 n = parallelQueue.size() + 1
    // 其中 parallel Queue.size()为 n-1 条正常处理的车道，
    ...
    public void enParallelAction(String queueTopic, Action action) {
        IActionQueue actionQueue = parallelQueue.get(queueTopic);
        if (actionQueue == null) {
            try {
    ...
    public void removeParallelAction(String queueTopic) {
        ...
        parallelQueue.remove(queueTopic);
    }
