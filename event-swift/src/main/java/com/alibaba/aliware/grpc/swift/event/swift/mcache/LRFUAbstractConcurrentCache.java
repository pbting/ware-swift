...
public abstract class LRFUAbstractConcurrentCache<K,V> extends AbstractConcurrentCache<K,V> {
	 ...
	 * key和访问次数据映射的hashmap
	 */
	private ConcurrentHashMap<K, Float> keyCacheRataMap = new ConcurrentHashMap<K, Float>();

	/**
	 * 访问映射字段：次数和这一类key的映射排序分类处理
	 */
	private ConcurrentHashMap<Float, Map<K, SRUKey>> cacheRateMapList = new ConcurrentHashMap<Float, Map<K, SRUKey>>();
	...
	public void itemPut(K key) {
		if (keyCacheRataMap.containsKey(key)) {// 已经包含该key，则该怎么办
			/**
			...
			if (ES == null) {
				// 表示系统的第一次
				ES = new ConcurrentHashMap<K,SRUKey>();
			...
			// 就算有一次也要记录下来
			this.keyCacheRataMap.put(key, cacheRate);
	...
	public void itemRetrieved(K key) {
		...
		if (!keyCacheRataMap.containsKey(key)){
			return;
		}
		
		// 1、获取当前key的一个响应比，
		Float cacheRate = keyCacheRataMap.get(key);
		// 获取这个访问次次数的一批key
		...
		if (nextLevelKeyMap == null) {
			nextLevelKeyMap = new ConcurrentHashMap<K,SRUKey>();
	...
	public void itemRemoved(K key) {
		...
		Float cacheRate = keyCacheRataMap.remove(key);
		// 获取这个访问次次数的一批key
		...
		try {
			for (Float cacheRate : this.cacheRateSort) {
				...
				if (levelLFUKeys != null && levelLFUKeys.isEmpty()) {// 这个时候表明这个访问层已经清空，则跳到下一层
					...
					if(isRemove){
						iter.remove();
						keyCacheRataMap.remove(removeKey);
					}
					break;
				}
			}
...
//				this.currentCacheRate = Math.abs(Float.valueOf(this.count * this.count) / (float) Math.sqrt(factor));
				this.currentCacheRate = Math.abs(Float.valueOf(this.count * this.count) / (float) factor);
