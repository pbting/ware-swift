...
import com.alibaba.aliware.grpc.swift.event.swift.disruptor.DisruptorParallelQueueExecutor.RunnableEvent;
import com.alibaba.aliware.grpc.swift.event.swift.parallel.action.AbstractParallelActionExecutor;
import com.alibaba.aliware.grpc.swift.event.swift.parallel.action.Action;
import com.alibaba.aliware.grpc.swift.event.swift.parallel.action.ActionBridge;
...
public class DisruptorParallelActionExecutor extends AbstractParallelActionExecutor {
	...
	private Disruptor<ActionEvent> builderDisruptor(int ringBufferSize) {
		Disruptor<ActionEvent> disruptor = new Disruptor<ActionEvent>(
	...
	public void stop() {
		...
		for (Disruptor<DisruptorParallelQueueExecutor.RunnableEvent> disruptor : disruptors) {
			disruptor.shutdown();
		}
	...
	public void executeOneTimeAction(Action action) {
		...
		IActionQueue<Action> actionQueue = new DisruptorParallelActionQueue(
				disruptors[ringBufferExecutorIndexChooser.allocator()], this.translator);
